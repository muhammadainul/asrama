jQuery(document).on('click', '.openDeleteAssetConfirmation', function() {
    ...
    jQuery('.deleteAsset').html("<form id='deleteConfirm' method='post' action='/asset/delete'>" +
        ...
        " <button type='submit' id='deleted' class='openActionDeleteAssetConfirmation btn btn-primary btn-sm' name='deleted' data-toggle='modal'>" +
        "<i class='fa fa-check' aria-hidden='true'></i> Delete</button></form>")
    jQuery("#deleteAssetConfirmation").modal('show')

    jQuery(".openActionDeleteAssetConfirmation").click(function (){
        jQuery("#actionDeletedAssetConfirmation").modal('show')
        jQuery("#deleteAssetConfirmation").modal('hide')
        setTimeout(function (){
