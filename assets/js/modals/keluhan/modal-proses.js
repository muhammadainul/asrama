jQuery(document).on('click', '.openActionProsesKeluhan', function() {
    ...
    jQuery('.deleteKamar').html("<form id='prosesKeluhan' method='post' action='/keluhan/proses'>" +
        ...
        "<i class='fa fa-check' aria-hidden='true'></i> Proses</button></form>")
    jQuery("#proseKeluhanConfirmation").modal('show')

    jQuery(".openActionConfirmation").click(function (){
        jQuery("#actionProcessedKeluhanConfirmation").modal('show')
        jQuery("#proseKeluhanConfirmation").modal('hide')
        setTimeout(function (){
            window.location.href='/keluhan'
        }, 2000)
    })
});