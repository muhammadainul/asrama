jQuery(document).on('click', '.openDeleteConfirmation', function() {
    ...
    jQuery('.deleteAsrama').html("<form id='deleteConfirm' method='post' action='/asrama/delete'>" +
    ...
    jQuery("#deleteAsramaConfirmation").modal('show')

    jQuery(".openActionConfirmation").click(function (){
        jQuery("#actionDeletedConfirmation").modal('show')
        jQuery("#deleteAsramaConfirmation").modal('hide')
        setTimeout(function (){
            window.location.href='/asrama'
        }, 2000)
    })
});